...
interface MaloofShareModalProps {
  isOpen: boolean
  onClose: () => void
  entryName: string
  entryType: string
...
export default function MaloofShareModal({ 
  isOpen, 
  onClose, 
  entryName, 
  entryType, 
  ...
  const handleFacebookShare = () => {
    const shareText = `Check out "${entryName}" (${entryType}) on Berwel - A Website for Libyan Music`
  ...
  const handleInstagramShare = () => {
    ...
    handleCopyLink()
    alert(`Instagram sharing: Copy the link and share it in your Instagram story or post!\n\nSuggested caption: Check out "${entryName}" (${entryType}) on Berwel - A Website for Libyan Music`)
  }
      ...
      <DialogContent className="bg-black border border-gray-700 text-white max-w-md">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gray-900 rounded-lg p-4 text-center">
            <h3 className="text-white font-bold text-xl mb-1">{entryName}</h3>
            <p className="text-gray-300">{entryType}</p>
          </div>
